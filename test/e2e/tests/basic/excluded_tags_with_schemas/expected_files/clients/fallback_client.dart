...
class FallbackClient {
  ...
  Future<GetApiV1NoTagsResponse> getNoTags() async {
    final response = await _gio.get(
      '${_baseUrl ?? ''}/api/v1/no-tags',
    );
    return GetApiV1NoTagsResponse.fromJson(
        jsonDecode(response.body) as Map<String, dynamic>);
  }

  Future<GetApiV1EmptyTagsResponse> getEmptyTags() async {
    final response = await _gio.get(
      '${_baseUrl ?? ''}/api/v1/empty-tags',
    );
    return GetApiV1EmptyTagsResponse.fromJson(
        jsonDecode(response.body) as Map<String, dynamic>);
  }
}
