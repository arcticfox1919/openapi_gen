...
import 'package:openapi_gen/src/generator/templates/dart_import_dto_template.dart';
...
String dartDartMappableDtoTemplate(
  ...
  final className = isUnion ? _applySealedNaming(originalClassName) : originalClassName;
  ...
  final hasCompleteMapping =
      isDiscriminatorVariant && dataClass.discriminatorValue!.propertyValue == originalClassName;
  ...
  final serializerClass = useFlutterCompute ? _generateFlutterComputeSerializer(className) : '';
...
String getParameters(UniversalComponentClass dataClass) {
  ...
  // in the parent class
  final parameters =
      dataClass.parameters.where((it) => it.name != dataClass.discriminator?.propertyName).toList();
...
String _parametersToString(List<UniversalType> parameters) {
  ...
  return sortedByRequired
      .mapIndexed(
        (i, e) => '${indentation(4)}${_required(e)}this.${e.name}${getDefaultValue(e)},',
      )
      .join('\n');
}
...
String _generateClassBody(UniversalComponentClass dataClass, String className,
    bool useMultipartFile, bool isUnion, bool dartMappableConvenientWhen, bool isSimpleDataClass,
    [String? fallbackUnion]) {
  if (!isUnion) {
...
''';
  ...
  if (dataClass.undiscriminatedUnionVariants case final variants? when variants.isNotEmpty) {
    return _generateUndiscriminatedUnionBody(
        className, variants, useMultipartFile, dartMappableConvenientWhen, fallbackUnion);
  }
...
String _generateUndiscriminatedUnionBody(String className, Map<String, Set<UniversalType>> variants,
    bool useMultipartFile, bool dartMappableConvenientWhen,
    [String? fallbackUnion]) {
  return '''
${indentation(2)}const $className();
...
String _generateUndiscriminatedUnionClasses(
    String className, Map<String, Set<UniversalType>> variants, bool useMultipartFile,
    [String? fallbackUnion]) {
  return '''
...
String _generateDiscriminatorHelper(String className, Discriminator discriminator,
    [String? fallbackUnion]) {
...
/// These functions support Flutter compute isolate-based serialization.
...
String _deserializerExtensionName(String className) =>
    className.endsWith('Sealed') ? '${className}Deserializer' : '${className}SealedDeserializer';
...
String _getMappableClassAnnotation(
    UniversalComponentClass dataClass, String className, String? fallbackUnion) {
...
Set<String> _getAllImports(
  ...
  if (!isUnion) {
    ...
    imports.removeWhere((import) {
      ...
      final isUsedByClass =
          usedTypes.contains(import) || usedTypes.any((type) => type.contains(import));
...
String _generateWrapperClasses(UniversalComponentClass dataClass, String className,
    bool useMultipartFile, String? fallbackUnion) {
  // Handle undiscriminated unions
  if (dataClass.undiscriminatedUnionVariants?.isNotEmpty ?? false) {
    return _generateUndiscriminatedUnionClasses(
        className, dataClass.undiscriminatedUnionVariants!, useMultipartFile, fallbackUnion);
  }
  ...
  if (dataClass.discriminator != null &&
      _isCompleteDiscriminatorMapping(dataClass.discriminator!)) {
    final wrappers =
        _generateDiscriminatedWrapperClasses(dataClass, className, useMultipartFile, fallbackUnion);
    return wrappers;
  }
...
String _generateDiscriminatedWrapperClasses(UniversalComponentClass dataClass, String className,
    bool useMultipartFile, String? fallbackUnion) {
  final discriminator = dataClass.discriminator!;
