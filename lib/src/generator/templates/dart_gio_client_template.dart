...
///
/// The produced class uses `Gio` method calls directly (no annotation-based code
/// generation).  Constructor accepts a `Gio` instance and an optional base URL
...
String dartGioClientTemplate({
  ...
  // dart:convert is required whenever we call jsonDecode.
  if (needsConvert) {
...
String _buildMethod(
  ...
  // Method signature — named-parameter block is only emitted when needed.
...
String _decodeResponse(UniversalType returnType, String dartType) {
  ...
  if (isList) {
    if (prim) {
      ...
      return '(jsonDecode(response.body) as List)'
          ' .cast<${_singleItemDartType(returnType)}>()';
    } else {
  ...
  if (prim) {
    return 'jsonDecode(response.body) as $dartType';
  }

  // Custom class
  final cls = returnType.type.toPascal;
  return '$cls.fromJson(jsonDecode(response.body) as Map<String, dynamic>)';
}
