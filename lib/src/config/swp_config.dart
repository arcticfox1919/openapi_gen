...
import 'package:openapi_gen/src/config/config_exception.dart';
...
class SWPConfig {
  ...
  factory SWPConfig.fromYaml(
    ...
    if (outputDirectory == null) {
      if (rootConfig == null) {
        ...
        throw ConfigException(
          ...
          "Add the 'output_directory' parameter under 'openapi_gen:' or set it for each schema.",
        );
      }
    ...
    final defaultContentType =
        yamlMap['default_content_type'] as String? ?? rootConfig?.defaultContentType;
    ...
    final enumsParentPrefix =
        yamlMap['enums_parent_prefix'] as bool? ?? rootConfig?.enumsParentPrefix;
    ...
    final language =
        rawLanguage == null ? rootConfig?.language : ProgrammingLanguage.fromString(rawLanguage);
    ...
    final rawHttpClient = yamlMap['http_client']?.toString();
    if (rawHttpClient != null) {
      httpClient = HttpClient.fromString(rawHttpClient);
    } else if (rootConfig?.httpClient != null) {
    ...
    final putClientsInFolder =
        yamlMap['put_clients_in_folder'] as bool? ?? rootConfig?.putClientsInFolder;
    ...
    final markFilesAsGenerated =
        yamlMap['mark_files_as_generated'] as bool? ?? rootConfig?.markFilesAsGenerated;
    ...
    if (rawReplacementRulesForRawSchema != null) {
      ...
      replacementRulesForRawSchema = List.from(rootConfig!.replacementRulesForRawSchema);
    }
    ...
    final dartMappableConvenientWhen = yamlMap['dart_mappable_convenient_when'] as bool? ??
    ...
    final excludePathsYaml = yamlMap['exclude_paths'] as YamlList?;
    List<String>? excludePathsList;
    if (excludePathsYaml != null) {
      excludePathsList = [];
      for (final p in excludePathsYaml) {
        if (p is! String) {
          throw const ConfigException(
            "Config parameter 'exclude_paths' values must be List of String.",
          );
        }
        excludePathsList.add(p);
      }
    } else if (rootConfig?.excludePaths case final paths?) {
      excludePathsList = List.from(paths);
    }
    ...
    final inferRequiredFromNullable =
        yamlMap['infer_required_from_nullable'] as bool? ?? rootConfig?.inferRequiredFromNullable;

    final useFlutterCompute =
        yamlMap['use_flutter_compute'] as bool? ?? rootConfig?.useFlutterCompute;

    final generateUrlsConstants =
        yamlMap['generate_urls_constants'] as bool? ?? rootConfig?.generateUrlsConstants;
    ...
    return SWPConfig._(
      ...
      httpClient: httpClient ?? dc.httpClient,
      ...
      replacementRulesForRawSchema: replacementRulesForRawSchema ?? dc.replacementRulesForRawSchema,
      ...
      inferRequiredFromNullable: inferRequiredFromNullable ?? dc.inferRequiredFromNullable,
      useFlutterCompute: useFlutterCompute ?? dc.useFlutterCompute,
      includePaths: includePathsList ?? dc.includePaths,
      excludePaths: excludePathsList ?? dc.excludePaths,
  ...
  /// DART ONLY
  /// Optional. HTTP client package to use for generated REST clients.
  /// Supported values: [HttpClient.gio], [HttpClient.dio].
  ...
  /// {@template exclude_paths}
  /// Optional. Set excluded paths.
  ///
  /// Also supports wildcard paths (e.g. `/path/*/update` or `/path/**`)
  ///
  /// Endpoints matching these paths will not be included in the generated clients.
  /// **NOTE: [includePaths] takes priority — if a path matches [includePaths] it is
  /// always included regardless of [excludePaths].**
  /// {@endtemplate}
